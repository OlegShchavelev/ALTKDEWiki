...
const props = computed(() => {
    ...
    var gallery = frontmatter.value.gallery[params.id]
    var images = [] 
    

    if (gallery.images) {
        for (let image of Object.keys(gallery.images)){
            var obj = {} 

            // Перехват изображений в билд //
            if (image.includes('https:')||image.includes('http:')){
                obj['src'] = image
            } else {
                obj['src'] = new URL(`/${route.path.slice(1) + image.slice(2)}`, import.meta.url).href
            }
            
            // Обработка остальных ключей //
            for (let key of Object.keys(gallery['images'][image])){
                obj[key] = gallery['images'][image][key]
            }
            images.push(obj)
        }
...
<template>
    ...
    <silent-box :gallery=props.images :lazy-loading=props.lazy></silent-box>
</template>
...
#silentbox-overlay__tool-buttons {
    text-align: center;
    margin-top: 1rem;
    width: 100%;
}
#silentbox-overlay__tool-buttons a {
    color: #fff;
    position: relative;
...
#silentbox-overlay__tool-buttons a:hover {
    color: #58e8d2;
    border-color: #58e8d2;
}
#silentbox-overlay__tool-buttons a:hover.download:before {
    background: #58e8d2;
}
#silentbox-overlay__tool-buttons a:hover.download:after {
    border-color: #58e8d2;
}
#silentbox-overlay__tool-buttons a.download {
    padding-right: 1.8rem;
}
#silentbox-overlay__tool-buttons a.download:before {
    content: " ";
    display: block;
    width: 1px;
    height: 9px;
...
#silentbox-overlay__tool-buttons a.download:after {
    content: " ";
    display: block;
    position: absolute;
    right: 0.9rem;
...
#silentbox-overlay__close-button .icon:hover:before,
#silentbox-overlay__close-button .icon:hover:after,
#silentbox-overlay__close-button .icon:focus:before,
#silentbox-overlay__close-button .icon:focus:after {
    background: #58e8d2;
    left: 25%;
    width: 50%;
}
...
#silentbox-overlay__arrow-buttons .arrow-previous:hover,
#silentbox-overlay__arrow-buttons .arrow-previous:focus {
...
#silentbox-overlay__arrow-buttons .arrow-next:hover,
#silentbox-overlay__arrow-buttons .arrow-next:focus {
    animation-duration: 1s;
    animation-iteration-count: infinite;
...
#silentbox-overlay .silentbox-video__embed:active,
#silentbox-overlay .silentbox-video__embed:focus,
#silentbox-overlay .silentbox-video__embed:hover {
    outline: none;
}
