...
Для упрощения процесса установки проприетарного драйвера графических карт NVIDIA возможно использование [интерфейса EPM](/package-manager/epm/).  Данный скрипт включает в себя **наиболее полную первоначальную настройку**, включая [активацию wayland](#сессия-wayland) и решения проблемы с [неизвестным монитором](#«неизвестныи-монитор»-в-настроиках-дисплеев-в-сессии-wayland). 
...
::: info
При установке драйверов через [EPM](#способ-1-через-epm) настройка под wayland уже будет произведена. Если Вы установили таким способом, дальнейшая настройка не требуется.
:::
